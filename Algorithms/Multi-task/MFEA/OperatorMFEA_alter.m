classdef OperatorMFEA_alter < Operator
        ...
        function [offspring, calls] = generate(population, Tasks, rmp, mu, mum)
            ...
            for i = 1:ceil(length(population) / 2)
                ...
                if (population(p1).skill_factor == population(p2).skill_factor) || rand < rmp
                    ...
                    % mutate
                    offspring(count) = OperatorMFEA.mutate(offspring(count), max([Tasks.dims]), mum);
                    offspring(count + 1) = OperatorMFEA.mutate(offspring(count + 1), max([Tasks.dims]), mum);
                    % imitate
                    p = [p1, p2];
                    ...
                    for x = 1:2
                        ...
                        % crossover
                        offspring(count + x - 1) = OperatorMFEA.crossover(offspring(count + x - 1), population(p(x)), population(idx), cf);
                        % mutate
                        offspring(count + x - 1) = OperatorMFEA.mutate(offspring(count + x - 1), max([Tasks.dims]), mum);
                        % imitate
