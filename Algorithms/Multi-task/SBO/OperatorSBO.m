    ...
    methods (Static)
        function offspring = generate(population, mu, mum)
            ...
            for i = 1:ceil(length(population) / 2)
                ...
                offspring(count) = OperatorSBO.crossover(offspring(count), population(p1), population(p2), cf);
                offspring(count + 1) = OperatorSBO.crossover(offspring(count + 1), population(p2), population(p1), cf);

                offspring(count) = OperatorSBO.mutate(offspring(count), length(population(1).rnvec), mum);
                offspring(count + 1) = OperatorSBO.mutate(offspring(count + 1), length(population(1).rnvec), mum);
