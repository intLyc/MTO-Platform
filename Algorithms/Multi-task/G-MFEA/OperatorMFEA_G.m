classdef OperatorMFEA_G < Operator
    ...
    methods (Static)
        function [offspring, calls] = generate(population, Tasks, rmp, mu, mum, transfer)
            ...
            for i = 1:ceil(length(population) / 2)
                ...
                if (population(p1).skill_factor == population(p2).skill_factor)
                    % crossover
                    population(p1).TDec = population(p1).Dec;
                    population(p2).TDec = population(p2).Dec;
                ...
                elseif rand < rmp
                    % crossover
                    population(p1).TDec = population(p1).Dec + transfer{population(p1).skill_factor, population(p2).skill_factor};
                    population(p2).TDec = population(p2).Dec + transfer{population(p2).skill_factor, population(p1).skill_factor};
                    offspring(count) = OperatorMFEA_G.crossover(offspring(count), population(p1), population(p2), cf);
                    offspring(count + 1) = OperatorMFEA_G.crossover(offspring(count + 1), population(p2), population(p1), cf);
                    offspring(count).Dec = offspring(count).Dec - transfer{population(p1).skill_factor, population(p2).skill_factor};
                    offspring(count + 1).Dec = offspring(count + 1).Dec - transfer{population(p2).skill_factor, population(p1).skill_factor};
                    % imitate
                    p = [p1, p2];
                    ...
                    for x = 1:2
                        % mutate
                        offspring(count + x - 1) = OperatorMFEA_G.mutate(population(p(x)), max([Tasks.Dim]), mum);
                        % imitate
        ...
        function object = crossover(object, p1, p2, cf)
            ...
            object.Dec = 0.5 * ((1 + cf) .* p1.TDec + (1 - cf) .* p2.TDec);
        end
