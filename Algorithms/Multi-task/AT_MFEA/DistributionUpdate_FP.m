...
function [mu, Sigma] = DistributionUpdate_FP(mu_old, Sigma_old, population, no_of_tasks)
    ...
    for i = 1:no_of_tasks
        ...
        for j = 1:length(population)
            if population(j).skill_factor == i
                fitness = [fitness; population(j).factorial_costs(i) + population(j).constraint_violation(i)];
