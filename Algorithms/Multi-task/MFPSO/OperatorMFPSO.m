classdef OperatorMFPSO < Operator
    ...
    methods (Static)
        function [population, calls] = generate(population, Tasks, rmp, w, c1, c2, c3, gbest)
            Individual_class = class(population(1));
            for i = 1:length(population)
                population(i) = OperatorMFPSO.velocityUpdate(population(i), gbest, rmp, w, c1, c2, c3, length(Tasks));
                population(i) = OperatorMFPSO.positionUpdate(population(i));
                population(i) = OperatorMFPSO.pbestUpdate(population(i));
            end
