classdef OperatorMTEA_AD < Operator
    ...
    methods (Static)
        function offspring = generate(population, mu, mum)
            ...
            for i = 1:ceil(length(population) / 2)
                ...
                offspring(count) = OperatorMTEA_AD.crossover(offspring(count), population(p1), population(p2), cf);
                offspring(count + 1) = OperatorMTEA_AD.crossover(offspring(count + 1), population(p2), population(p1), cf);

                offspring(count) = OperatorMTEA_AD.mutate(offspring(count), length(population(1).rnvec), mum);
                offspring(count + 1) = OperatorMTEA_AD.mutate(offspring(count + 1), length(population(1).rnvec), mum);
