    ...
    methods (Static)
        function [offspring, calls] = generate(population, Tasks, taski, mu, mum)
            ...
            for i = 1:ceil(length(population) / 2)
                ...
                offspring(count) = OperatorSRE.mutate(offspring(count), length(population(1).rnvec), mum);
                offspring(count + 1) = OperatorSRE.mutate(offspring(count + 1), length(population(1).rnvec), mum);
