    ...
    methods (Static)
        function [offspring, calls] = generate(population, mu, mum)
            ...
            for i = 1:ceil(length(population) / 2)
                ...
                offspring(count) = OperatorEMEA_GA.mutate(offspring(count), length(population(1).rnvec), mum);
                offspring(count + 1) = OperatorEMEA_GA.mutate(offspring(count + 1), length(population(1).rnvec), mum);
