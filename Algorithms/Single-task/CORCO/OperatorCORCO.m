classdef OperatorCORCO < Operator
        ...
        function [offspring, calls] = generate(population, Task, F, CR, weights)
            ...
            for i = 1:length(population)
                ...
                if rand() < 0.5
                    ...
                    offspring(i) = OperatorCORCO.mutate_rand_to_best(offspring(i), population(best), population(x1), population(x2), population(x3), F(randi(length(F))));
                    offspring(i) = OperatorCORCO.crossover(offspring(i), population(i), CR(randi(length(CR))));
                else
                    % current-to-rand
