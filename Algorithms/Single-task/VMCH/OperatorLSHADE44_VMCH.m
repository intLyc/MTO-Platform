classdef OperatorLSHADE44_VMCH < Operator
            ...
            for i = 1:length(population)
                ...
                switch population(i).st
                    case 1 % pbest + bin
                        ...
                        offspring(i) = OperatorLSHADE44_VMCH.mutate_rand_to_pbest(offspring(i), population(i), population(pbest), population(x1), union(x2));
                        offspring(i) = OperatorLSHADE44_VMCH.crossover(offspring(i), population(i));
                    case 2 % pbest + exp
                        pbest = pop_pbest(randi(length(pop_pbest)));
                        x1 = randi(length(population));
                        while x1 == i || x1 == pbest
                        ...
                        offspring(i) = OperatorLSHADE44_VMCH.mutate_rand_to_pbest(offspring(i), population(i), population(pbest), population(x1), union(x2));
                        offspring(i) = OperatorLSHADE44_VMCH.crossoverExp(offspring(i), population(i));
                    case 3 % randrl + bin
                        ...
                        offspring(i) = OperatorLSHADE44_VMCH.mutate_rand(offspring(i), population(x1), population(x2), population(x3), population(i).F);
                        offspring(i) = OperatorLSHADE44_VMCH.crossover(offspring(i), population(i));
                    case 4 % randrl + exp
                        A = randperm(length(population), 4);
                        A(A == i) = []; idx = A(1:3);
