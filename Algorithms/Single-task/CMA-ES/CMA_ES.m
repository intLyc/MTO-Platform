    ...
    methods
        function data = run(obj, Tasks, RunPara)
            ...
            for sub_task = 1:length(Tasks)
                ...
                cs = (mu_eff + 2) / (Task.Dim + mu_eff + 5);
                ds = 1 + cs + 2 * max(sqrt((mu_eff - 1) / (Task.Dim + 1)) - 1, 0);
                ENN = sqrt(Task.Dim) * (1 - 1 / (4 * Task.Dim) + 1 / (21 * Task.Dim^2));
                % Covariance update parameters
                cc = (4 + mu_eff / Task.Dim) / (4 + Task.Dim + 2 * mu_eff / Task.Dim);
                c1 = 2 / ((Task.Dim + 1.3)^2 + mu_eff);
                cmu = min(1 - c1, 2 * (mu_eff - 2 + 1 / mu_eff) / ((Task.Dim + 2)^2 + 2 * mu_eff / 2));
                hth = (1.4 + 2 / (Task.Dim + 1)) * ENN;
                % Initialization
                ...
                while fnceval_calls < sub_eva
                    ...
                    % Sample solutions
                    Pstep = zeros(length(population), Task.Dim);
                    for i = 1:sub_pop
                        Pstep(i, :) = mvnrnd(zeros(1, Task.Dim), C);
                        population(i).Dec = Mdec + sigma .* Pstep(i, :);
                        population(i).Dec(population(i).Dec > Task.Ub) = Task.Ub(population(i).Dec > Task.Ub);
                        population(i).Dec(population(i).Dec < Task.Lb) = Task.Lb(population(i).Dec < Task.Lb);
                    end
                    [population, calls] = evaluate(population, Task, 1, 'real');
                    fnceval_calls = fnceval_calls + calls;

                    [~, rank] = sort([population.Obj]);
                    bestObj_now = population(rank(1)).Obj;
                    if generation == 1
                        bestObj = bestObj_now;
                        bestDec_temp = population(rank(1)).Dec;
                    else
