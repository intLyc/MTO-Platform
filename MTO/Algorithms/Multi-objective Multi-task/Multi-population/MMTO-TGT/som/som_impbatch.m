function [sMap,D,sTrain] = som_impbatch(sMap, D, varargin)

%SOM_IMPBATCH  Use batch algorithm to train the Imputation SOM.
%
% [sM,sT] = som_impbatch(sM, D, [argID, value, ...])
% 
%  sM     = som_impbatch(sM,D);
%  sM     = som_impbatch(sM,sD,'radius',[10 3 2 1 0.1],'tracking',3);
%  [M,sT] = som_impbatch(M,D,'ep','msize',[10 3],'hexa');
%
...
%
% For more help, try 'type som_impbatch' or check out online documentation.
...
%
%  sM = som_impbatch(sM,D);
%  sM = som_impbatch(sM,sD);
%  sM = som_impbatch(...,'argID',value,...);
%  sM = som_impbatch(...,value,...);
%  [sM,sT] = som_impbatch(M,D,...);
%
% DESCRIPTION
%
...
% Vatanen, T., ..., "cite to WSOM 2012 paper"
%    TODO
%
%
% KNOWN BUGS
%
% Function does not work correctly for a map with a single unit. 
...
% temporary:
dataWithMissing = any(missing,2);
missingValuesTmp = missing(dataWithMissing,:);
...
for t = 1:trainlen,
  ...
  % for debugging
%   som_grid(sMap,'Coord',M,'Markersize',2,'Linecolor','k','Surf',M(:,3))
%   clf;
%   plot3(D(:,1), D(:,2), D(:,3), 'k+');hold on;
%   plot3(D(dataWithMissing,1), D(dataWithMissing,2), D(dataWithMissing,3), 'r+')
%   drawnow;
%   pause(0.05)
...
%   tmp = M(bmus(dataWithMissing),:);
%   D(missingValues) = tmp(missingValuesTmp);
  ...
  % Construct D using "expected values" for NaNs.
  DTmp= D;
  tmp = M(bmus(dataWithMissing),:);
  DTmp(missing) = tmp(missingValuesTmp);
  
  S = H*(P*DTmp); 
  A = H*(P*ones(size(D)));

  % An alternative way to construct P*D
  %PD = zeros(munits, dim);
  %Dtmp = D;
  %for unit = 1:munits
  %  tmp = repmat(M(unit,:), [dlen 1]);
  %  Dtmp(missing) = tmp(missing);
  %  PD(unit,:) = P(unit,:)*Dtmp;
  %end
