classdef MTEA_PAE < Algorithm
% <Multi-task> <Single-objective> <None/Constrained> <Year: 2025>
...
methods
    ...
    function run(Algo, Prob)
        ...
        for t = 1:Prob.T
            ...
            [~, rank] = sortrows([Pop{t}.CVs, Pop{t}.Objs], [1, 2]);
            Pop{t} = Pop{t}(rank);
        end
        Arc = Pop;
        DisPop = Pop;
        ...
        while Algo.notTerminated(Prob, Pop)
            ...
            if Algo.TNum > 0 && mod(Algo.Gen, Algo.TGap) == 0
                ...
                % Knowledge Transfer
                for t = 1:Prob.T
                    [~, rank] = sortrows([DisPop{t}.CVs, DisPop{t}.Objs], [1, 2]);
                    DisPop{t} = DisPop{t}(rank);
                    Arc{t} = [Arc{t}, Pop{t}];
                    [~, rank] = sortrows([Arc{t}.CVs, Arc{t}.Objs], [1, 2]);
                    Arc{t} = Arc{t}(rank);
                end
            ...
            for t = 1:Prob.T
                ...
                temp = [Pop{t}, Off{t}];
                [Pop{t}, rank] = Selection_Elit(Pop{t}, Off{t});
                Failed = temp(rank(Prob.N + 1:end));
