classdef CMT4 < Problem
    ...
    methods
        ...
        function setTasks(obj)
            ...
            obj.Fnc{1} = @(x)Rastrigin1(x, 1, 0 * ones(1, obj.D(1)), -20 * ones(1, obj.D(1)));
            obj.Lb{1} = -50 * ones(1, obj.D(1));
            obj.Ub{1} = 50 * ones(1, obj.D(1));

            obj.Fnc{2} = @(x)Sphere1(x, 1, 0 * ones(1, obj.D(2)), 30 * ones(1, obj.D(2)));
            obj.Lb{2} = -100 * ones(1, obj.D(2));
            obj.Ub{2} = 100 * ones(1, obj.D(2));
        end
    end
end
