classdef CMT9 < Problem
    ...
    methods
        ...
        function setTasks(obj)
            ...
            obj.Fnc{1} = @(x)Rastrigin4(x, 1, -10 * ones(1, obj.D(1)), 0 * ones(1, obj.D(1)));
            obj.Lb{1} = -50 * ones(1, obj.D(1));
            obj.Ub{1} = 50 * ones(1, obj.D(1));

            obj.Fnc{2} = @(x)Schwefel2(x, 1, 0 * ones(1, obj.D(2)), 100 * ones(1, obj.D(2)));
            obj.Lb{2} = -500 * ones(1, obj.D(2));
            obj.Ub{2} = 500 * ones(1, obj.D(2));
        end
    end
end
