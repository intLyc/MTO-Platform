...
function [PopObj, PopCVs] = dtlz3(PopDec, M)
%------------------------------- Copyright --------------------------------
...
PopObj = (1 + g) .* fliplr(cumprod([ones(size(PopDec, 1), 1), cos(PopDec(:, 1:M - 1) * pi / 2)], 2)) .* [ones(size(PopDec, 1), 1), sin(PopDec(:, M - 1:-1:1) * pi / 2)];
