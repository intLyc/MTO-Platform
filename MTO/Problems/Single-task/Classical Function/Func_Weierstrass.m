classdef Func_Weierstrass < Problem
...
methods
    function Prob = Func_Weierstrass(varargin)
        Prob = Prob@Problem(varargin);
        Prob.maxFE = 1000 * Prob.D;
        Prob.Lb{1} = -0.5 * ones(1, Prob.D);
        Prob.Ub{1} = 0.5 * ones(1, Prob.D);
    end
    ...
    function setTasks(Prob)
        ...
        Prob.T = 1;
        Prob.Fnc{1} = @(x)Weierstrass(x, 1, 0, 0);
    end
end
end
