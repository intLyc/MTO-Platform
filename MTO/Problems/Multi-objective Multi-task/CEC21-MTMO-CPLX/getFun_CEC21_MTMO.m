    ...
    case 'F11'
        ...
        for i = 1:length(x)
            ...
            if abs(z) <= 500
                gz = z * sin(abs(z)^0.5);
            elseif z > 500
                gz = (500 - mod(z, 500)) * sin(abs(500 - mod(z, 500))^0.5) - ((z - 500)^2) / (10000 * length(x));
            else
                gz = (mod(abs(z), 500) - 500) * sin(abs(500 - mod(abs(z), 500))^0.5) - ((z + 500)^2) / (10000 * length(x));
            end
            prod1 = prod1 + gz;
        end
        Gfunction = 418.9829 * length(x) - prod1;
    case 'F15'
        x = x - shiftVector;
        x = x .* (5.0/100);
        x = rotationMatrix * x';
