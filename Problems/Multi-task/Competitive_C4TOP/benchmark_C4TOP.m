function Tasks = benchmark_C4TOP(index)
    ...
    switch index
        case 1 % The combination of f1 and f2
            Tasks(1).dims = D(1); % dimensionality of Task 1
            Tasks(1).fnc = @(x)Rastrigin(x, M_matrix{1}, O_matrix{1}, fbias(1));
            Tasks(1).Lb = Xmin(1) * ones(1, Tasks(1).dims); % Upper bound of Task 1
            Tasks(1).Ub = Xmax(1) * ones(1, Tasks(1).dims); % Lower bound of Task 1

            Tasks(2).dims = D(2); % dimensionality of Task 2
            Tasks(2).fnc = @(x)Griewank(x, M_matrix{2}, O_matrix{2}, fbias(2));
            Tasks(2).Lb = Xmin(2) * ones(1, Tasks(2).dims); % Upper bound of Task 2
            Tasks(2).Ub = Xmax(2) * ones(1, Tasks(2).dims); % Lower bound of Task 2

            Tasks(3).dims = D(3); % dimensionality of Task 3
            Tasks(3).fnc = @(x)Sphere(x, M_matrix{3}, O_matrix{3}, fbias(3));
            Tasks(3).Lb = Xmin(3) * ones(1, Tasks(3).dims); % Upper bound of Task 3
            Tasks(3).Ub = Xmax(3) * ones(1, Tasks(3).dims); % Lower bound of Task 3

            Tasks(4).dims = D(4); % dimensionality of Task 4
            Tasks(4).fnc = @(x)Rosenbrock(x, M_matrix{4}, O_matrix{4}, fbias(4));
            Tasks(4).Lb = Xmin(4) * ones(1, Tasks(4).dims); % Upper bound of Task 4
            Tasks(4).Ub = Xmax(4) * ones(1, Tasks(4).dims); % Lower bound of Task 4
