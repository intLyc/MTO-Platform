classdef CMT1 < Problem
    ...
    methods
        ...
        function Tasks = getTasks(obj)
            Tasks(1).Dim = obj.Dim;
            Tasks(1).Fnc = @(x)Griewank1(x, 1, 0 * ones(1, obj.Dim), -40 * ones(1, obj.Dim));
            Tasks(1).Lb = -100 * ones(1, obj.Dim);
            Tasks(1).Ub = 100 * ones(1, obj.Dim);

            Tasks(2).Dim = obj.Dim;
            Tasks(2).Fnc = @(x)Rastrigin1(x, 1, 0 * ones(1, obj.Dim), 20 * ones(1, obj.Dim));
