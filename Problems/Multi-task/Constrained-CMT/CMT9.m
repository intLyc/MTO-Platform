classdef CMT9 < Problem
    ...
    methods
        ...
        function Tasks = getTasks(obj)
            Tasks(1).Dim = obj.Dim;
            Tasks(1).Fnc = @(x)Rastrigin4(x, 1, -10 * ones(1, obj.Dim), 0 * ones(1, obj.Dim));
            Tasks(1).Lb = -50 * ones(1, obj.Dim);
            Tasks(1).Ub = 50 * ones(1, obj.Dim);

            Tasks(2).Dim = obj.Dim;
            Tasks(2).Fnc = @(x)Schwefel2(x, 1, 0 * ones(1, obj.Dim), 100 * ones(1, obj.Dim));
