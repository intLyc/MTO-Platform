classdef CMT3 < Problem
    ...
    methods
        ...
        function Tasks = getTasks(obj)
            Tasks(1).Dim = obj.Dim;
            Tasks(1).Fnc = @(x)Ackley2(x, 1, 42.096 * ones(1, obj.Dim), 40 * ones(1, obj.Dim));
            Tasks(1).Lb = -50 * ones(1, obj.Dim);
            Tasks(1).Ub = 50 * ones(1, obj.Dim);

            Tasks(2).Dim = obj.Dim;
            Tasks(2).Fnc = @(x)Schwefel1(x, 1, 0 * ones(1, obj.Dim), 400 * ones(1, obj.Dim));
            Tasks(2).Lb = -500 * ones(1, obj.Dim);
            Tasks(2).Ub = 500 * ones(1, obj.Dim);
        end
    end
end
