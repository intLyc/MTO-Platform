classdef CMT8 < Problem
    ...
    methods
        ...
        function Tasks = getTasks(obj)
            Tasks(1).Dim = obj.Dim;
            Tasks(1).Fnc = @(x)Griewank2(x, 1, 0 * ones(1, obj.Dim), -30 * ones(1, obj.Dim));
            Tasks(1).Lb = -100 * ones(1, obj.Dim);
            Tasks(1).Ub = 100 * ones(1, obj.Dim);

            Tasks(2).Dim = obj.Dim;
            Tasks(2).Fnc = @(x)Weierstrass3(x, 1, 0 * ones(1, obj.Dim), 0.2 * ones(1, obj.Dim));
            Tasks(2).Lb = -0.5 * ones(1, obj.Dim);
            Tasks(2).Ub = 0.5 * ones(1, obj.Dim);
        end
    end
end
