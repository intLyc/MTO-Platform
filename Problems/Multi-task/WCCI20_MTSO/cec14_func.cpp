...
void cec14_test_func(double *, double *, int, int, int, int, int);

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	int m, n, func_num, benchmark_id, task_id;
	double *f, *x;
	if ((nrhs < 4) || (nlhs < 1))
	{
		mexPrintf("usage: f = cec14_func(x, func_num, benchmark_id, task_id);\n");
		mexErrMsgTxt("example: f= cec14_func([3.3253000e+000, -1.2835000e+000]', 1, 1, 1);");
	}
	n = mxGetM(prhs[0]); // dimension
	// if (n != 50)
	// {
	// 	mexPrintf ("usage: f = cec14_func(x, func_num, benchmark_id, task_id);\n");
	// 	mexErrMsgTxt ("Error: Test functions are only defined for D = 50.");
	// }
	m = mxGetN(prhs[0]);
	x = mxGetPr(prhs[0]);
	...
	f = mxGetPr(plhs[0]);
	cec14_test_func(&x[0], &f[0], n, m, func_num, benchmark_id, task_id);
}

void cec14_test_func(double *x, double *f, int nx, int mx, int func_num, int benchmark_id, int task_id)
{

	//     printf("func_id = %d benchmark_id = %d  task_id = %d init = %d\n", func_num, benchmark_id, task_id, ini_flag);
	int cf_num = 10, i, j;
	if (ini_flag == 1)
	{
		if ((n_flag != nx) || (func_flag != func_num) || (benchmark_flag != benchmark_id) || (task_flag != task_id))
		{
			ini_flag = 0;
		}
	}
	...
	if (ini_flag == 0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "./Problems/Multi-task/WCCI20_MTSO/Tasks/benchmark_%d/matrix_%d", benchmark_id, task_id);
		//        printf("matrix: %s\n", FileName);
		...
		//         printf("\n");
		/* Load shift_data */
		sprintf(FileName, "./Problems/Multi-task/WCCI20_MTSO/Tasks/benchmark_%d/bias_%d", benchmark_id, task_id);
		//        printf("shift: %s\n", FileName);
		fpt = fopen(FileName, "r");
		if (fpt == NULL)
		{
		...
		else
		{
			...
			for (i = 0; i < cf_num - 1; i++)
			{
				for (j = 0; j < nx; j++)
				{
					fscanf(fpt, "%le", &OShift[i * nx + j]);
				}
				fscanf(fpt, "%*[^\n]%*c");
			}
			for (j = 0; j < nx; j++)
			{
				fscanf(fpt, "%le", &OShift[(cf_num - 1) * nx + j]);
			}
		}
		fclose(fpt);
		//         for(int i = 0; i < nx; ++i) {
		...
		{
			sprintf(FileName, "./Problems/Multi-task/WCCI20_MTSO/Tasks/shuffle/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName, "r");
			if (fpt == NULL)
			{
				printf("\n Error: Cannot open input file for reading \n");
