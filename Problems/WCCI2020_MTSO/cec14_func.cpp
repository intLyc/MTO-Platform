...
void cec14_test_func(double *x, double *f, int nx, int mx, int func_num, int benchmark_id, int task_id)
{
	...
	if (ini_flag == 0)
	{
		...
		/* Load Matrix M*/
		sprintf(FileName, "./Problems/WCCI2020_MTSO/Tasks/benchmark_%d/matrix_%d", benchmark_id, task_id);
		//        printf("matrix: %s\n", FileName);
		...
		//         printf("\n");
		/* Load shift_data */
		sprintf(FileName, "./Problems/WCCI2020_MTSO/Tasks/benchmark_%d/bias_%d", benchmark_id, task_id);
		//        printf("shift: %s\n", FileName);
		fpt = fopen(FileName, "r");
		if (fpt == NULL)
		{
		...
		{
			sprintf(FileName, "./Problems/WCCI2020_MTSO/Tasks/shuffle/shuffle_data_%d_D%d.txt", func_num, nx);
			fpt = fopen(FileName, "r");
			if (fpt == NULL)
			{
				printf("\n Error: Cannot open input file for reading \n");
